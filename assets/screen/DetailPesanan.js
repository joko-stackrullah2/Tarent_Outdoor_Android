...
const DetailPesanan = ({navigation, route}) => {
  ...
  const createPDF = async () => {
    if (await isPermitted()) {
        let options = {
            html: HTMLCode,
            fileName: `bukti-pesan-${IDOrder+'#'+NamaPenyewa}`,
            directory: 'Download',
        };
        ...
        const fileName = `bukti-pesan-${IDOrder+'#'+NamaPenyewa}.pdf`;
        const downloadDest = `${DownloadDir}/${fileName}`;

        try {
            await RNFS.moveFile(sourcePath, downloadDest);
        ...
        //   console.log('status_transaksi : ' + status_transaksi);
          ...
          if (
            ...
            var transaction_time =
              response.data.result.payment_response.transaction_time;
          ...
          var tanggal_sewa =
            ...
            '-' +
            tanggal_transaksi.getFullYear() +
            ' Pukul: ' +
            tanggal_transaksi.getHours() +
            ':' +
            tanggal_transaksi.getMinutes() +
            ':' +
            tanggal_transaksi.getSeconds() +
            ' WIB';
          ...
          for (let index = 0; index < daftar_produk.length; index++) {
            ...
            daftar_barang += `<tr style="border:0.5px solid black;">
                    ...
                    <td style="text-align: right; padding-left: 10px; padding-right: 10px; padding-top:5px; padding-bottom:5px; font-size: 14px;">${FormatRupiahGo(
                      harga_sewa,
                    )}</td>
                </tr>`;
          }
    ...
    <View
      ...
      <View style={{marginLeft: 10, flex: 6}}>
        ...
        <Text style={{color: 'black', fontFamily: 'Inter-Bold', fontSize: 14}}>
          {FormatRupiahGo(harga_sewa)} x {jumlah_pesanan}
        </Text>
      </View>
      <View
        style={{
        ...
        <Text style={{color: 'black', fontFamily: 'Inter-Bold', fontSize: 14}}>
          {FormatRupiahGo(harga_sewa * jumlah_pesanan)}
        </Text>
      </View>
    </View>
  );
  ...
  const VerifikasiPembayaranTunai = async () => {
    ...
    await axios
      .get(url + 'api/payment/verifikasi', {
        params: {
          id_order: IDOrder,
        },
      })
  ...
  const VerPengembalianBarang = async () => {
    ...
    await axios
      .post(url + 'api/verifikasi_pengembalian', ParameterUrl)
      .then(response => {
        // console.log(response.data);
        if (response.data.status == true) {
          GetDaftarBarang(IDOrder);
  ...
  const HalamanHTMLStruk = (Base64QRCode) => {
    ...
    <body>
        <div class="container">
            <img class="qr-code" src="data:image/jpeg;base64,${Base64QRCode}" alt="QR Code">
            <h1 style="text-align: center;">Detil Pesanan</h1>
            <table>
                <tr>
                    <th>No</th>
                    <th>Nama Barang</th>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>${FormatRupiahGo(TotalPembayaran)}</td>
                </tr>
            </table>
            <table>
                <tr>
            ...
            <p class="thank-you">Terimakasih telah menyewa barang di Tarent Outdoor.</p>
        </div>
    </body>
    ...
    return (
      <View>
        <View style={{marginTop: 10, flexDirection: 'row'}}>
          <View style={{flex: 1}}>
            <Text
              style={{fontFamily: 'Inter-Bold', fontSize: 14, color: 'black'}}>
              Total
            </Text>
            <TouchableOpacity onPress={() => copyToClipboard(TotalPembayaran)}>
              <Text
                ...
                {FormatRupiahGo(TotalPembayaran)}
              </Text>
            </TouchableOpacity>
            <Text
              style={{fontFamily: 'Inter-Bold', fontSize: 14, color: 'black'}}>
              order ID #{IDOrder + IDUser}
            </Text>
          </View>
        ...
        {VirtualAcoount != '' ? (
          <View>
            ...
            {/* Cara Pembayaran BCA */}
            <TouchableOpacity
              onPress={() => OpenCaraPembayaran(MetodePembayaran)}
              style={{
                flexDirection: 'row',
                marginTop: 10,
                alignItems: 'center',
            ...
            {CaraPembayaranBCA ? (
              <View style={{marginTop: 10}}>
                {/* ATM BCA */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBCA('ATM')}
                ...
                {/* Klik BCA */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBCA('KBCA')}
                ...
                {/* m-BCA */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBCA('mBCA')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
            ...
            {CaraPembayaranBNI ? (
              <View style={{marginTop: 10}}>
                {/* ATM BNI */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBNI('ATM')}
                  style={{flexDirection: 'row'}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {ATMBNI ? (
                  <View>
                    <Text
                      ...
                      1. Select others on the main menu.
                    </Text>
                    <Text
                      style={{
                        fontFamily: 'Inter-Bold',
                ...
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBNI('IBBNI')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {/* Mobile Banking BNI */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBNI('MBBNI')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
            ...
            {CaraPembayaranBRI ? (
              <View style={{marginTop: 10}}>
                {/* ATM BRI */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBRI('ATM')}
                  style={{flexDirection: 'row'}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {/* Internet Banking BNI */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBRI('IBBRI')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {/* Mobile Banking BRI */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranBRI('MBBRI')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
        ...
        {BillCode != '' ? (
          <View>
            ...
            <TouchableOpacity
              onPress={() => setCaraPembayaranMandiri(!CaraPembayaranMandiri)}
              style={{
                flexDirection: 'row',
                marginTop: 10,
                alignItems: 'center',
            ...
            {CaraPembayaranMandiri ? (
              <View style={{marginTop: 10}}>
                {/* ATM Mandiri */}
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranMandiri('ATM')}
                  style={{flexDirection: 'row'}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {ATMMandiri ? (
                  <View>
                    <Text
                      ...
                      1. Select Pay/Buy on the main menu.
                    </Text>
                    <Text
                      style={{
                        fontFamily: 'Inter-Bold',
                ...
                <TouchableOpacity
                  onPress={() => CekCaraPembayaranMandiri('IB')}
                  style={{flexDirection: 'row', marginTop: 10}}>
                  <View style={{flex: 1}}>
                    <Text
                      style={{
                ...
                {InternetBankingMandiri ? (
                  <View>
                    <Text
                      ...
                      1. Select Payment on the main menu.
                    </Text>
                    <Text
                      style={{
                        fontFamily: 'Inter-Bold',
        ...
        {UrlQRCodeGoPay != '' ? (
          <View>
            ...
            <TouchableOpacity
              onPress={() => setCaraPembayaranGoPay(!CaraPembayaranGoPay)}
              style={{
                flexDirection: 'row',
                marginTop: 10,
                alignItems: 'center',
              ...
              <View style={{marginTop: 10}}>
                <View>
                  <Text
                    ...
                    1. Buka aplikasi Gojek atau e-wallet lain Anda.
                  </Text>
                  <Text
                    style={{
                      fontFamily: 'Inter-Bold',
                  ...
                  <Text
                    ...
                    3. Konfirmasi pembayaran pada aplikasi.
                  </Text>
                  <Text
                    style={{
                      fontFamily: 'Inter-Bold',
        ...
        {MetodePembayaran == 'Tunai' ? (
          <View>
            {!StatusTransaksi ? (
              <TouchableOpacity
                onPress={() => VerifikasiPembayaranTunai()}
                style={{
                  borderRadius: 10,
                  width: '100%',
                  paddingVertical: 10,
        ...
        {StatusTransaksi ? (
          <View style={{marginTop: 10}}>
            ...
            <View style={{alignItems: 'center', marginTop: 10}}>
              <QRCode value={IDOrder + IDUser} getRef={c => GetImageQR(c)} />
            </View>
          </View>
        ) : (
          <View></View>
          ...
          {MetodePembayaran == 'GoPay' ? (
            <View>
              <View
                ...
                <Image
                  source={GopayIcon}
                  style={{width: 80, height: 40, marginRight: 20}}
                  resizeMode={'contain'}
                />
              </View>
              {DetilPembayaran()}
            ...
            {StatusSewa == null ? (
              <View
                ...
                <Text
                  ...
                  Anda dapat melakukan verifikasi pengembalian barang dengan
                ...
                <Text
                  ...
                  Waktu pengembalian barang adalah:
                </Text>
                <Text
                  style={{
                    fontFamily: 'Inter-Bold',
                ...
                <Text
                  ...
                  Apakah pelanggan terlambat mengembalikan barang atau barang
                  mengalami kerusakan?
                </Text>
                <Text
                  style={{
                  ...
                  Masukkan jumlah denda di bawah ini
                </Text>
                <View
                  style={{
                    borderWidth: 1,
              ...
              <View
                ...
                <Text
                  ...
                  Total Denda : {FormatRupiahGo(TotalDenda)}
                </Text>
                <Text
                  style={{
                    fontFamily: 'Inter-Bold',
